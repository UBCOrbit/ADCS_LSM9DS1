...
Re-written to plain C + use I2C under TMS570 by Jasper Chan
...
void LSM9DS1_initMag(imu_t* imu)
{
	...
	// SIM - I2C mode selection (0:write-only, 1:read/write enable)
...
void LSM9DS1_configInt(imu_t* imu, interrupt_select_t intrpt, uint8_t generator,
	                     h_lactive_t activeLow, pp_od_t pushPull)
{
	...
	LSM9DS1_xgWriteByte(imu, intrpt, generator);
...
{
	return LSM9DS1_I2CwriteByte(imu->settings.device.mAddress, subAddress, data);
}
